...
export const Dropdown = ({ dropDownList, innitial = "none", handleClick }) => {
    ...
    return <div className=" text-white flex flex-col ">
        ...
        }} className={` w-fit text-[16px] cursor-pointer flex flex-row items-center bg-green-400 py-[3px] px-[6px]  shadow-md rounded-t-[5px] ease-out duration-300 ${active ? "" : "rounded-b-[5px] hover:py-[4px] hover:px-[7px]"}`}>
        ...
        {/* Dropped down part */}
        <div className={` ${!active ? "h-0 w-0 overflow-clip p-0" : "flex overflow-scroll h-fit py-2 px-1"} text-[14px] ease-out transition-all duration-300  bg-green-500 max-h-[150px] rounded-b-[5px] flex-col`}>
            {list.map((el) => {
